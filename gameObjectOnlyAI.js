...
GameAI.prototype.GameLoop = function(n) {
  if (this.gameIsAlive) {
    ...
    var res = gen[n].compute(closestAst(this.AIShip, nbSensors, this.asteroids));
    ...
    if (res[2] < 0.5) {
      if (this.AIShip.currentCooldown >= this.AIShip.cooldown) {
        this.lasers.push(new Laser(this.AIShip.pos, this.AIShip.heading));
        //pew.play();
    ...
    for (var i = 0; i < this.asteroids.length; i++) {
      ...
      if(gameDisplayMode) this.asteroids[i].show();
    ...
    for (var i = this.lasers.length - 1; i >= 0; i--) {
      ...
      if(gameDisplayMode) this.lasers[i].render();
    ...
    if(gameDisplayMode) this.AIShip.render();
    this.AIShip.turn();
    this.AIShip.update();
    this.AIShip.edges();
  }
